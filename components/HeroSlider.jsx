...
export default function HeroSlider({ movies }) {
  ...
  const { data: trailerData, error } = useSWR(
    selectedMedia
      ? `https://api.themoviedb.org/3/${selectedMedia.media_type}/${selectedMedia.id}/videos?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=en_US`
      : null,
    fetcher,
  )
        ...
        {movies.map((media) => (
          <SwiperSlide key={media.id}>
            <div className='relative h-screen w-full'>
              ...
              <div
                className='absolute flex inset-0 items-center sm:items-end p-4 sm:p-8 md:p-20
